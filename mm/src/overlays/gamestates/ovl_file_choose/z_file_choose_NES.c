...
void FileSelect_DrawWindowContents(GameState* thisx) {
    ...
    for (fileIndex = 0; fileIndex < 3; fileIndex++, temp += 28) {
        if (fileIndex < (CVarGetInteger("gEnhancements.Saving.FileSlot3", true) ? 3 : 2)) {
            gDPPipeSync(POLY_OPA_DISP++);
    ...
    for (i = 0; i < 3; i++, temp += 16) {
        if (i < (CVarGetInteger("gEnhancements.Saving.FileSlot3", true) ? 3 : 2)) {
            // draw file button
    ...
    // draw file info
    for (fileIndex = 0; fileIndex < (CVarGetInteger("gEnhancements.Saving.FileSlot3", true) ? 3 : 2); fileIndex++) {
